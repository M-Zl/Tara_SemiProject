...
public class EncryptUtil {
	/*
		암호화의 방향성
		  1) 단방향 : 암호화된 평문은 다시 복화화 할 수 없는 암호화 방식이다. (SHA(Secure Hash Algorithm2)(<-요즘이걸 많이쓴다), MD5(Message-Digest algorithm 5))
		  2) 양방향 : 암호화된 평문을 다시 복호화 할 수 있는 암호화 방식이다. 
			...
			**https ssl 암호화방식..? 부분 찾아보기

		SHA(Secure Hash Algorithm2) - 사용자의 암호는 사용자만 알아야함.
		  1) SHA-256 
	...
	*/
	...
	public static String oneWayEnc(String message, String algorithm) {
		...
		try {
			...
			MessageDigest md = MessageDigest.getInstance(algorithm); //해시값으로 변환해주는 역할
